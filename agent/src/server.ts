...
Key tables:
- patient_summary: start here, one row per patient with visit counts and costs
- encounters: filter by ENCOUNTERCLASS (emergency, inpatient, ambulatory, urgentcare, wellness)
- conditions: active when STOP IS NULL (clinical + SDOH)
- medications: active when STOP IS NULL
- observations: PRAPARE social screenings (housing, food, transport, stress)
- claims_transactions: financial claims, join on PATIENTID (not PATIENT)
- careplans: active care plans when STOP IS NULL

Rules:
- Only generate SQL SELECT queries
- Always include a LIMIT clause in SQL queries
- Never show raw JSON to users; provide a clear summary or table
- Use LIKE with LOWER() for name matching due to numeric suffixes in synthetic names
      ...
      tools: {
        ...
        queryDatabase: tool({
          description:
            ...
            "Use patient_summary first for cost/risk triage. " +
          ...
          execute: async ({ sql }) => {
            ...
            if (!res.ok || data.success === false) {
              return {
                ...
                error:
                  data.error ||
                  `Database query failed with status ${res.status}`
              };
            }
